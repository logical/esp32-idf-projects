...
//void i2c_init(){}; // join i2c bus (address optional for master)
...
//    Wire.endTransmission(true);
    ...
    uint8_t address=FMTX_CMD_ADDRESS;
	...
	i2c_master_write_byte(cmd, dt, true);
	...
	if (espRc == ESP_OK) {
		...
		ESP_LOGE(TAG, "write failed. code: 0x%.2X", espRc);
	}
...
*/
    ...
    i2c_master_read_byte(cmd, &dt, true);
    i2c_master_stop(cmd);

    esp_err_t espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
		ESP_LOGI(TAG, "read success: %d",dt);
	} else {
		ESP_LOGE(TAG, "read failed. code: 0x%.2X", espRc);
	}
	i2c_cmd_link_delete(cmd);

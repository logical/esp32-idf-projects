...
///for speaker only
#define SPK_GPIO_WS (14)
#define SPK_GPIO_CLK (27)
#define SPK_GPIO_DOUT (26)

//for microphone
#define MIC_GPIO_WS      (33)
#define MIC_GPIO_CLK     (32)
#define MIC_GPIO_DIN      (35)
...
//      8khz is the best i could get using hci
...
{
    int16_t frames_buffer[2 * sizeof(int16_t) * NUM_FRAMES_TO_SEND];
  ...
  while (frame_index < frames)
  {
    ...
    {
      int left_sample = (uint16_t)(volume * samples[frame_index * 2]);
      int right_sample = (uint16_t)(volume * samples[frame_index * 2 + 1]);
    ...
    i2s_channel_write(tx_chan, frames_buffer, frames_to_send* sizeof(int16_t) * 2, &bytes_written, portMAX_DELAY);
...
///THIS PAUSES THE MUSIC
        if(stop_playing){
          ...
          while(stop_playing)vTaskDelay(pdMS_TO_TICKS(100));
        }
      }

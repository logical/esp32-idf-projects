...
uint32_t twosCompToDec(uint32_t two_compliment_val)
{
    // [0x0000; 0x7FFF] corresponds to [0; 32,767]
    // [0x8000; 0xFFFF] corresponds to [-32,768; -1]
    // int16_t has the range [-32,768; 32,767]

//    uint16_t sign_mask = 0x8000;//16 bit
   uint32_t sign_mask = 0x800000;//24 bit

    int32_t value=0;
//    two_compliment_val &=0x00ffffff;
    // if positive
    if ( (two_compliment_val & sign_mask) == 0 ) {
        value= two_compliment_val;
    //  if negative
    } else {
        // invert all bits, add one, and make negative
        value= -(~two_compliment_val + 1);
    }
    return value+0xdffffff;
}
...
//convert signed data from microphone to unsigned data
    size_t bytes = 0;
//    ESP_LOGI(BT_HF_TAG,"bytes %d",);

////////////////////////////////////////////////////////////////////////////////TODO
    uint32_t convBuff[sz];
    i2s_channel_read(rx_chan,convBuff , sz, &bytes, 0);

    for(int i=0; i < bytes/sizeof(uint32_t); i++){

        uint16_t data=twosCompToDec(convBuff[i]);
//        uint16_t data=(~convBuff[i]);
        *((uint16_t*)&p_buf[i*2])=data/4;

    }

//    i2s_channel_read(rx_chan,p_buf , sz, &bytes, 0);
...
/*
    ...
    i2s_channel_write(tx_chan, buf, sz, &bytes_written, portMAX_DELAY);

    ESP_LOGE(BT_HF_TAG, "rb send");

    esp_hf_client_outgoing_data_ready();
*/
//static uint8_t control_num=0;
        size_t bytes = 0;
/*
////////////convert the data
        int16_t outbuf[sz/sizeof(uint16_t)];
        for(int i=0;i<sz/sizeof(uint16_t);i+=sizeof(uint16_t)){
            int32_t data=(buf[i]-32767)*VOLUME_ADJUST;
            outbuf[i]=data+32767;
        }
*/
    i2s_channel_write(tx_chan, buf, sz, &bytes, 0);
//        if (control_num < 5) { control_num++; } else { control_num = 0; esp_hf_client_outgoing_data_ready(); }
        esp_hf_client_outgoing_data_ready();
